...
1. On the **Explorer** pane on the left, in the **...** menu for the **Files** node, select **New subfolder**
...
1. In the navigation pane on the left, select your `fab_workspace`.

2. In the workspace, select **New item**, search for **Pipeline**

3. Create a new pipeline with the Name: `Ingest Sales Data`

4. In the pipeline editor canvas start with a blank canvas:
...
2. In the pane below the canvas select the **Source** tab.
...
10. Select **Test connection** to verify the connection works.
...
2. In the **Choose Destination** dialog box, select *OneLake catalog* on the left hand menu.

    Then select your `fab_lakehouse`
...
1. On the **Home** menu or the **Run** menu, click **Stop Session**

2. In the **Explorer** pane on the left, in the **...** menu for **Tables** select **Refresh**

    - Verify that a **sales** table has been created.

3. In the notebook menu bar, use the ⚙️ **Settings** icon to view the notebook settings.
...
3. Select the **Delete data** activity, and in the pane below the design canvas, set the following properties:
    ...
    - **Source**:
        - **Connection**: Browse all, and select your lakehouse by clicking **OneLake catalog**
